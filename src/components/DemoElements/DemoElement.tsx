            ...
            default:
                out = <div className={'w-sm-full w-8-min'}>
                    <div style={{display: 'none'}}
                         className={"pad dsp-none"}>.{moduleAttribute} .{moduleRule} .{cssFabricClassName}</div>
                    {DrawType()}
                </div>
        }
        return out;
...
const DemoElementText = (props: IDemoElementGridProps) => {
    ...
    return <div className={' pad-2 overflow-none scale-w-16'}>
        ...
        <p className={cssFabricClassName + " text-ellipsis"}>This {cssFabricClassName} text is from cssfabric a css sass
            framework</p>
    </div>
}
